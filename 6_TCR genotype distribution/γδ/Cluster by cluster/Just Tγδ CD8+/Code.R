setwd("C:/Esmaeil/CeliacProject/CeliacProject/6_TCR genotype distribution/γδ/Cluster by cluster/Just Tγδ CD8+")
library(dplyr)
library(circlize)
library(ggplot2)
library(ggalluvial)
...
full_metadata = full_metadata[full_metadata$cluster == "Tγδ CD8+" , ]
...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    title(main = paste("TRGV-TRDV pairing - Tγδ CD8+ -", p), line = 0, cex.main = 1.5)
...
p <- ggplot(alluvial_data,
  ...
  labs(title = "Proportion of TRDV Types per Diagnosis - Tγδ CD8+",
       x = "Diagnosis",
       y = "Percentage (%)") +
...
p <- ggplot(alluvial_data_TRGV,
  ...
  scale_fill_manual(values = dark_colors,
                    breaks = c("TRGV2", "TRGV3", "TRGV4",
                               ...
                               "TRGV9", "TRGV10", "TRGV11")) +
  scale_y_continuous(labels = percent_format(scale = 1)) +
  labs(title = "Proportion of TRGV Types per Diagnosis - Tγδ CD8+",
       x = "Diagnosis",
       y = "Percentage (%)") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5))
...
ggsave("TRGV_alluvial_percentages_V2.pdf", plot = p, width = 10, height = 5, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
