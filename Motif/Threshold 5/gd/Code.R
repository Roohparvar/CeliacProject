...
metadata <- metadata[!is.na(metadata$g_cdr3) & !is.na(metadata$d_cdr3), ]

# Keep only unique pairs of (g_cdr3, d_cdr3)
pairs <- unique(metadata[, c("g_cdr3", "d_cdr3")])

dist_g <- stringdistmatrix(pairs$g_cdr3, pairs$g_cdr3, method = "lv")
dist_d <- stringdistmatrix(pairs$d_cdr3, pairs$d_cdr3, method = "lv")

dist_matrix <- dist_g + dist_d
dist_matrix <- as.matrix(dist_matrix)

rownames(dist_matrix) <- paste(pairs$g_cdr3, pairs$d_cdr3, sep = "+")
colnames(dist_matrix) <- paste(pairs$g_cdr3, pairs$d_cdr3, sep = "+")
...
degree_matrix <- degree_matrix %>% left_join(cluster_map, by = c("Clone" = "cdr_Full_gd"))
...
Eigenvector <- Eigenvector %>%
  left_join(cluster_map, by = c("Node" = "cdr_Full_gd"))
...
topEigenvector_nodes <- topEigenvector_nodes %>%
  left_join(full_metadata %>% 
              select(cdr_Full_gd, imm_receptor_Esmaeil, clone_size_ab),
            by = c("Node" = "cdr_Full_gd")) %>%
...
topEigenvector_nodes <- topEigenvector_nodes %>%
  separate(Node, into = c("g_cdr3", "d_cdr3"), sep = "\\+", remove = FALSE)
...
p <- ggseqlogo(aligned_seqs, method = "prob") +
  ggtitle("Logo plots of the CDR3g") +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16)  # center the title
  )

# Save as high-quality PNG
ggsave("Logo plots of the CDR3g.png", plot = p,
...
seqs <- topEigenvector_nodes$d_cdr3
seqs <- unique(seqs)
# Convert character vector to AAStringSet (required by msa)
seqs <- AAStringSet(seqs)
...
p <- ggseqlogo(aligned_seqs, method = "prob") +
  ggtitle("Logo plots of the CDR3d") +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16)  # center the title
  )

# Save as high-quality PNG
ggsave("Logo plots of the CDR3d.png", plot = p,
       width = 7, height = 3, dpi = 300, bg = "white")
...
p <- ggseqlogo(aligned_seqs, method = "prob") +
  ggtitle("Logo plots of the CDR3 Aberrant g only") +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16)  # center the title
  )

# Save as high-quality PNG
ggsave("Logo plots of the CDR3g only.png", plot = p,
       width = 7, height = 3, dpi = 300, bg = "white")
