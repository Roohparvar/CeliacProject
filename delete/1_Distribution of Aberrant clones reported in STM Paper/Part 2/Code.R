...
legend_df <- data.frame(
  ...
  color = c(
    "#eb2525", "#f8a41d", "#6bccdf", "#81509f", "#69bc48",
    "#b45e3d", "#9aaf9e", "#ca563f", "#f4d3c2", "#e9d41f"
  ),
  shape = c(rep("circle", 6), rep("triangle", 4)),
  stringsAsFactors = FALSE
)


# Add legend labels to metadata
...
full_metadata$legend_label <- factor(full_metadata$legend_label, levels = c(circle_labels, triangle_labels))
...
main_plot <- ggplot() +
  geom_point(data = full_metadata[full_metadata$color == "#CBCBCB", ],
             ...
             color = "#CBCBCB", size = 0.5, alpha = 0.5) +
  geom_point(data = full_metadata[full_metadata$color != "#CBCBCB", ],
             aes(x = scVI_with_hvg_UMAP_1, y = scVI_with_hvg_UMAP_2,
  ...
  labs(
    title = "Distribution of Aberrant clones reported in STM",
    x = "UMAP 1",
    y = "UMAP 2"
  )

# Legends for circle and triangle shapes
p_circles <- ggplot() +
...
# Combine UMAP and legends
legend_combined <- p_circles + p_triangles + patchwork::plot_layout(ncol = 2)
