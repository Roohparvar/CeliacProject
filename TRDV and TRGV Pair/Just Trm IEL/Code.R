...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    title(main = paste("TRGV-TRDV pairing - Trm IEL Clusters -", p), line = 0.3, cex.main = 0.9)
...
p <- ggplot(diagnosis_summary, aes(x = Diagnosis, y = percent, fill = TRDV)) +
  ...
  labs(x = "Diagnosis", y = "Percentage",
       title = "Proportion of TRDV types per Diagnosis - All Tgd Clusters - Trm IEL Clusters") +
...
p <- ggplot(diagnosis_summaryTRGV, aes(x = Diagnosis, y = percent, fill = TRGV)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "Diagnosis", y = "Percentage",
       title = "Proportion of TRGV types per Diagnosis - Trm IEL Clusters") +
