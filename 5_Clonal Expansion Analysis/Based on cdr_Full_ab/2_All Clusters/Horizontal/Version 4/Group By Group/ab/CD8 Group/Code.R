...
full_metadata = full_metadata[full_metadata$cluster == "CD8 Mem" |
                                full_metadata$cluster == "CD8 Trm" |
                                full_metadata$cluster == "CD8 Cyt." , ]
...
p1 <- ggplot(df, aes(x = Diagnosis, y = perc, fill = clone_category)) +
  ...
  labs(
    ...
    fill = "Clone Size",
    title = "αβ Clonal Size Distribution by Diagnosis (Percentage) | CD8 Group"
  ) +
  white_theme +
  scale_x_discrete(expand = c(0,10)) +
  coord_cartesian(clip = "off")

ggsave("CD8 Group (Percentage).pdf", p1, width = 4, height = 2, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
ggsave("CD8 Group (Percentage).png", p1, width = 4, height = 2, dpi = 600, bg = "white")
...
p2 <- ggplot(df, aes(x = Diagnosis, y = n_cells, fill = clone_category)) +
  ...
  labs(
    ...
    y = "Number of Cells", 
    fill = "Clone Size",
    title = "αβ Clonal Size Distribution by Diagnosis (Raw Counts) | CD8 Group"
  ) +
  white_theme +
  scale_x_discrete(expand = c(0,10)) +
  coord_cartesian(clip = "off") 

ggsave("CD8 Group (Raw Counts).pdf", p2, width = 4, height = 2, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
ggsave("CD8 Group (Raw Counts).png", p2, width = 4, height = 2, dpi = 600, bg = "white")
