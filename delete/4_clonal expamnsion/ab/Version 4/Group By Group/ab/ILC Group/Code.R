...
full_metadata = full_metadata[full_metadata$cluster == "ILC1" |
                                full_metadata$cluster == "ILC2/ILTi" |
                                full_metadata$cluster == "ILC2/ILC3" , ]
...
p1 <- ggplot(df, aes(x = Diagnosis, y = perc, fill = clone_category)) +
  ...
  labs(
    ...
    fill = "Clone Size",
    title = "αβ Clonal Size Distribution by Diagnosis (Percentage) | ILC Group"
  ) +
  white_theme +
  scale_x_discrete(expand = c(0,10)) +
  coord_cartesian(clip = "off")

ggsave("ILC Group (Percentage).pdf", p1, width = 4, height = 2, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
ggsave("ILC Group (Percentage).png", p1, width = 4, height = 2, dpi = 600, bg = "white")
...
p2 <- ggplot(df, aes(x = Diagnosis, y = n_cells, fill = clone_category)) +
  ...
  labs(
    ...
    y = "Number of Cells", 
    fill = "Clone Size",
    title = "αβ Clonal Size Distribution by Diagnosis (Raw Counts) | ILC Group"
  ) +
  white_theme +
  scale_x_discrete(expand = c(0,10)) +
  coord_cartesian(clip = "off") 

ggsave("ILC Group (Raw Counts).pdf", p2, width = 4, height = 2, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
ggsave("ILC Group (Raw Counts).png", p2, width = 4, height = 2, dpi = 600, bg = "white")
