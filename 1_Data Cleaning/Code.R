...
#--------------------------------------------------------------------------------- Remove rows where cluster is DC, Macrophages, or Mast cells and imm_receptor_Esmaeil is not empty or NA
...
num_removed <- length(rows_to_remove)

full_metadata <- full_metadata[-rows_to_remove, ]



#--------------------------------------------------------------------------------- Remove rows where ig_hk or ig_hL is present but imm_receptor_Esmaeil is not "hkl"
rows_to_remove_igh <- which(
...
# فیلتر اینکه imm_receptor_Esmaeil نباید "hkl" باشد
rows_to_remove_final <- rows_to_remove_igh[
  full_metadata$imm_receptor_Esmaeil[rows_to_remove_igh] != "hkl"
]

# حذف سطرها
num_removed_igh <- length(rows_to_remove_final)
full_metadata <- full_metadata[-rows_to_remove_final, ]
