...
full_metadata = full_metadata[full_metadata$cluster == "Tgd" | full_metadata$cluster == "Trm IEL", ]
...
p1 <- ggplot(df, aes(x = Diagnosis, y = perc, fill = clone_category)) +
  ...
  labs(
    ...
    fill = "Clone Size",
    title = "γδ Clonal Size Distribution by Diagnosis (Percentage) | Tγδ and Trm IEL"
  ) +
  white_theme +
  scale_x_discrete(expand = c(0,10)) +
  coord_cartesian(clip = "off") 

ggsave("Tγδ and Trm IEL (Percentage).pdf", p1, width = 4, height = 2, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
ggsave("Tγδ and Trm IEL (Percentage).png", p1, width = 4, height = 2, dpi = 600, bg = "white")
...
p2 <- ggplot(df, aes(x = Diagnosis, y = n_cells, fill = clone_category)) +
  ...
  labs(
    ...
    y = "Number of Cells", 
    fill = "Clone Size",
    title = "γδ Clonal Size Distribution by Diagnosis (Raw Counts) | Tγδ and Trm IEL"
  ) +
  white_theme +
  scale_x_discrete(expand = c(0,10)) +
  coord_cartesian(clip = "off")

ggsave("Tγδ and Trm IEL (Raw Counts).pdf", p2, width = 4, height = 2, dpi = 600, bg = "white", device = cairo_pdf, family = "Arial Unicode MS")
ggsave("Tγδ and Trm IEL (Raw Counts).png", p2, width = 4, height = 2, dpi = 600, bg = "white")
