...
Healthy_healthy <- rowMeans(count_matrix[, Healthy_samples, drop = FALSE])
mean_RCD1 <- rowMeans(count_matrix[, RCD1_samples, drop = FALSE])


log2fc <- log2( (mean_RCD1 + 1) / (Healthy_healthy + 1) )
...
plot(
  ...
  xlab = "% difference TRDV gene use(RCD1 versus Healthy)",
  ylab = "-log10(p-value)",
  pch = 20,
  col = "black",
