...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    title(main = paste("TRGV-TRDV pairing - Tgd CD8+ -", p), line = 0.3, cex.main = 0.9)
...
p <- ggplot(diagnosis_summary, aes(x = Diagnosis, y = percent, fill = TRDV)) +
  ...
  labs(x = "Diagnosis", y = "Percentage",
       title = "Proportion of TRDV types per Diagnosis - Tgd CD8+") +
...
p <- ggplot(alluvial_data,
  ...
  labs(title = "Proportion of TRDV Types per Diagnosis - Tgd CD8+",
       x = "Diagnosis",
       y = "Percentage (%)") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5))
