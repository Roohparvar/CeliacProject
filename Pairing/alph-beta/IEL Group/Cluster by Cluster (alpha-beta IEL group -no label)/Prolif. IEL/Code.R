setwd("C:/Esmaeil/CeliacProject/CeliacProject/Pairing/alph-beta/IEL Group/Cluster by Cluster (alpha-beta IEL group -no label)/Prolif. IEL")
...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    title(main = paste("TRAV-TRBV pairing -", p), line = 0.3, cex.main = 0.9)
...
p <- ggplot(alluvial_data,
  ...
  labs(title = "Proportion of TRAV Types per Diagnosis",
       x = "Diagnosis",
       y = "Percentage (%)") +
