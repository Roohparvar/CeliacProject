...
p1 <- ggplot(df, aes(x = cluster, y = perc, fill = clone_category)) +
  ...
  labs(
    ...
    title = "Clonal Expansion of Aberrant αβ T Cells Across IEL Clusters in RCD-I (Percentage)"
  ) +
  white_theme
...
p2 <- ggplot(df, aes(x = cluster, y = n_cells, fill = clone_category)) +
  ...
  labs(
    ...
    title = "Clonal Expansion of Aberrant αβ T Cells Across IEL Clusters in RCD-I (Raw Counts)"
  ) +
  white_theme
