setwd("C:/Esmaeil/CeliacProject/CeliacProject/5_TCR gene pairing/γδ/Just Tγδ INSIG1+")
library(dplyr)
library(circlize)
library(ggplot2)
library(ggalluvial)
...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    title(main = paste("TRGV-TRDV pairing - Tγδ INSIG1+ -", p), line = 0.3, cex.main = 0.9)
