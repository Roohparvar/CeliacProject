setwd("C:/Esmaeil/CeliacProject/CeliacProject/6_TCR genotype distribution/γδ/Cluster by cluster/All Tγδ Clusters")
#------------------------------------------------------------------------------- libraries
library(dplyr)
library(circlize)
library(ggplot2)
...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    title(main = paste("All Tγδ Clusters -", p), line = 0, cex.main = 1.5)
...
p <- ggplot(alluvial_data,
  ...
  labs(title = "Proportion of TRDV Types per Diagnosis - All Tγδ Clusters",
       x = "Diagnosis",
       y = "Percentage (%)") +
...
p <- ggplot(alluvial_data_TRGV,
  ...
  scale_fill_manual(values = dark_colors,
                    breaks = c("TRGV2", "TRGV3", "TRGV4",
                               ...
                               "TRGV9", "TRGV10", "TRGV11")) +
  scale_y_continuous(labels = percent_format(scale = 1)) +
  labs(title = "Proportion of TRGV Types per Diagnosis - All Tγδ Clusters",
       x = "Diagnosis",
       y = "Percentage (%)") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5))
