...
for (p in Diagnosiss) {
  ...
  if (nrow(df) > 0) {
    ...
    ## --- PDF output ---
    pdf(file = paste0("Circos_", p, ".pdf"), width = 9, height = 8)  
    chordDiagram(df, 
                 order = sector_order,
                 transparency = 0.5,
    ...
    title(main = paste("TRGV-TRDV pairing - NK Tgd Cluster -", p), line = 0.3, cex.main = 0.9)
...
# Ensure all TRDV types exist for every Diagnosis
diagnosis_summary <- diagnosis_summary %>%
  complete(Diagnosis, TRDV = c("TRDV1", "TRDV2", "TRDV3"), fill = list(count = 0)) %>%
  group_by(Diagnosis) %>%
  mutate(total = sum(count),
         percent = round(100 * count / total, 2)) %>%
  ungroup() %>%
...
p <- ggplot(diagnosis_summary, aes(x = Diagnosis, y = percent, fill = TRDV)) +
  ...
  labs(x = "Diagnosis", y = "Percentage",
       title = "Proportion of TRDV types per Diagnosis - NK Tgd Cluster") +
...
p <- ggplot(alluvial_data,
  ...
  labs(title = "Proportion of TRDV Types per Diagnosis - NK Tgd Cluster",
       x = "Diagnosis",
       y = "Percentage (%)") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5))
...
p <- ggplot(diagnosis_summaryTRGV, aes(x = Diagnosis, y = percent, fill = TRGV)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(x = "Diagnosis", y = "Percentage",
       title = "Proportion of TRGV types per Diagnosis - NK Tgd Cluster") +
...
p <- ggplot(alluvial_data_TRGV,
  ...
  scale_fill_manual(values = dark_colors,
                    breaks = c("TRGV2", "TRGV3", "TRGV4",
                               ...
                               "TRGV9", "TRGV10")) +
  scale_y_continuous(labels = percent_format(scale = 1)) +
  labs(title = "Proportion of TRGV Types per Diagnosis - NK Tgd Cluster",
       x = "Diagnosis",
       y = "Percentage (%)") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5))
