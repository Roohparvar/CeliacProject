...
meta_RCD1 <- full_metadata %>% filter(Diagnosis == "RCD-I")

# 2. اضافه کردن ستون cluster اگر موجود نیست
if(!"cluster" %in% colnames(meta_RCD1)) {
  meta_RCD1$cluster <- as.character(Idents(merged_obj1)[rownames(meta_RCD1)])
}

# 3. فیلتر سلول‌هایی که TRBV == "TRBV7-2"
meta_RCD1_TRBV7_2 <- meta_RCD1 %>% filter(TRBV == "TRBV7-2")

# 4. شمارش سلول TRBV7-2 در هر کلاستر
count_TRBV7_2 <- meta_RCD1_TRBV7_2 %>%
  group_by(cluster) %>%
  summarise(count = n())

# 5. اضافه کردن کلاسترهای بدون TRBV7-2 (صفر)
all_clusters <- unique(meta_RCD1$cluster)
