setwd("C:/Esmaeil/CeliacProject/CeliacProject/Shanon/gd/diagnosis by diagnosis")
...
target_clusters <- c(
  "Th", "Tregs", "CD8 Mem", "IEL GZMK+", "Th17", "CD8 Cyt.", "CD8 Trm", 
  "Prolif. IEL", "B cells_1", "Plasmablast", "Th1 Mem", "DC", "ILC2/ILC3", 
  "CD4-CD8- IL10 ICOS", "CD4-CD8-", "ILC1", "Th2/Tfh", "Cyt. IEL", 
  "B cells MZB1+", "IEL CCL4+", "Plasma cells_2", "Macrophages", "Mast cells",
  "nIEL", "B cells_2", "B cells BAFFR", "Plasma cells_1", "ILC2/ILTi"
)
...
p_combined <- ggplot(cluster_Diagnosis_diversity, aes(x = cluster, fill = Diagnosis)) +
  geom_rect(aes(xmin = as.numeric(cluster) - 0.2,
                xmax = as.numeric(cluster) + 0.2,
                ymin = y_start,
                ymax = y_end)) +
  scale_fill_manual(values = c(
    "Healthy" = "#D83A8A",
