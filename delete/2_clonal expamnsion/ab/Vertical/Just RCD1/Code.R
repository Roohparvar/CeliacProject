...
metadata_filtered <- full_metadata %>%
  mutate(clone_size_ab = as.numeric(as.character(clone_size_ab)),  # convert table to numeric
         clone_size_ab = ifelse(cluster %in% target_clusters, 1, clone_size_ab))
...
p1 <- ggplot(df, aes(y = cluster, x = perc, fill = clone_category)) +
  ...
  labs(
    y = "Cluster", x = "Percentage of Cells", fill = "TCRαβ Clones",
...
ggsave("CloneSize_ab_Percentage_Vertical2.png", p1, width = 6, height = 10, dpi = 400, bg = "white")
...
p2 <- ggplot(df, aes(y = cluster, x = n_cells, fill = clone_category)) +
  ...
  scale_fill_manual(values = clone_colors) +
  labs(
    y = "Cluster", x = "Number of Cells", fill = "TCRαβ Clones",
...
ggsave("CloneSize_ab_RawCounts_Vertical2.png", p2, width = 6, height = 10, dpi = 400, bg = "white")
